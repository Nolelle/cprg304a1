...
public class AppDriver {
	 ...
	 * sorting operation. Accepts three command line arguments: -f filename:
	 * the input file containing shape data -t [h|v|a]: comparison type
	 * (height, volume, or area) -s [b|s|i|m|q|z]: sort algorithm (bubble,
	 * selection, insertion, merge, quick, or heap)
	 *
	...
	public static void main(String[] args) {
		...
		try {
			...
			Comparator<Shape> comparator = switch (compareType) {
				case 'h' ->
					null; // Use natural ordering (height)
				case 'a' ->
					new AreaComparator();
				case 'v' ->
	...
	/**
	 * Reads shapes from the input file and creates corresponding Shape
	 * objects. The first line of the file should contain the number of
	 * shapes. Each subsequent line should contain: shape_type height
	 * dimension where dimension is either radius (for Cylinder/Cone) or
	 * side length (for Prisms/Pyramid)
	 *
	 * @return array of Shape objects read from the file
	 * @throws FileNotFoundException if the input file cannot be found
	 * @throws IllegalArgumentException if the shape type is invalid or data
	 * is malformed
	 */
	private static Shape[] readShapesFromFile() throws FileNotFoundException {
		...
		for (int i = 0; i < numShapes; i++) {
			...
			shapes[i] = switch (shapeType.toLowerCase()) {
				case "cylinder" ->
					new Cylinder(height, dimension);
				case "cone" ->
					new Cone(height, dimension);
				case "pyramid" ->
					new Pyramid(dimension, height);
				case "squareprism" ->
					new SquarePrism(height, dimension);
				case "triangularprism" ->
					new TriangularPrism(height, dimension);
				case "pentagonalprism" ->
					new PentagonalPrism(dimension, height);
				case "octagonalprism" ->
					new OctagonalPrism(height, dimension);
				default ->
	...
	/**
	 * Prints the sorted array of shapes. Displays first element, every
	 * 1000th element, and last element.
	 *
